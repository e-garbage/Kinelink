...
#set min not working - placeholder command
@app.get("/p/setmin", description="Set Minimum position possible (limit switch) using SAP command")
async def p_setmin(
    addr: int = Query(..., description="Module (motor) Address 0-255"),
    pos: int = Query(..., description="position to go -100000 - 100000")
    ):
    ...
    return {"call from":"p_setmin","reply":reply, "api error":e}
...
async def p_setmax(
    ...
    if not (0<=addr<=255):
        ...
        e=(e or "")+f"Wrong input address. address set to {addr}. "
    reply = await motor_manager.sap(addr, 1, 0, pos)
    return {"call from":"p_setmax","reply":reply, "api error":e}
...
async def p_setaccel(
    ...
    if not (0<=addr<=255):
        addr=0
        e=(e or "")+f"Wrong input address. address set to {addr}. "
    reply = await motor_manager.sap(addr, 5, 0, accel)
