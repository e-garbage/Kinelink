...
class Writer(asyncio.Protocol):
    ...
    def tmcl_command_builder(self, addr, cmd, typ, value):
        data = struct.pack('>BBBi', addr, cmd, typ, value)
        checksum =sum(data[1:]) & 0XFF
        logging.debug(f"command {cmd} built for addr {addr} with value {value}")
    ...
    async def send(self):
        ...
        message=Writer.tmcl_command_builder(self,17,6,0,0)
...
reader = serial_asyncio.create_serial_connection(loop, reader_partial, '/dev/ttyUSB0', baudrate=115200)
writer = serial_asyncio.create_serial_connection(loop, Writer, '/dev/ttyUSB0', baudrate=115200)
